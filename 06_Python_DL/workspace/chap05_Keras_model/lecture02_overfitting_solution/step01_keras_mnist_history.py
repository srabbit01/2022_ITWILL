...
History : 훈련과 검증과정에서 발생하는 손실값/평가 결과 기억 기능 
 - 최적의 epoch size 결정: train vs val 손실(loss) 교차점 -> 이 이상 반복은 과적합
"""
...
model = Sequential()
# Layer 많고, 뉴런이 많을 수록 과적합(Overfitting) 위험 존재
...
Epoch 15/15
600/600 [==============================] - 1s 2ms/step
 - loss: 0.0111 - accuracy: 0.9963 - val_loss: 0.0946 - val_accuracy: 0.9786
'''
...
# loss vs val_loss : overfitting 시작점: 3 epochs
...
plt.legend(loc='best')
plt.show()
# 정확도 97% 정도인 지점 2 epochs가 최적임을 볼 수 있음
# Maximum 3 epochs 정도가 적절함
